    ...
    <AppLayout className="bg-slate-950 text-slate-100 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 lg:py-12 space-y-16 pb-32">
        <div className="relative h-[540px] w-full flex items-center justify-center overflow-hidden rounded-[50px] border border-white/10 bg-black/40 shadow-2xl glass3d">
           <div className={`absolute inset-0 transition-opacity duration-1000 ${isComplete ? 'opacity-40' : 'opacity-10'} bg-[radial-gradient(circle_at_50%_50%,#3b82f6,transparent)]`} />
           <div className="relative z-10 flex flex-col items-center text-center">
              <div className="size-80 border border-white/5 rounded-full flex items-center justify-center relative">
                <div className={`absolute inset-0 border-t-2 ${isComplete ? 'border-purple-500 shadow-[0_0_15px_#a855f7]' : 'border-blue-500/20'} rounded-full animate-spin-slow`} />
                <motion.div
                  whileHover={{ scale: 1.05 }}
                  className={`size-56 rounded-full bg-gradient-to-br from-cyanNeon/10 to-magentaPulse/10 border border-white/20 flex items-center justify-center backdrop-blur-3xl cursor-pointer transition-all duration-1000 ${isComplete ? 'shadow-[0_0_80px_rgba(255,0,255,0.4)]' : 'shadow-glow-lg hover:shadow-glow-xl'}`}
                  onClick={() => navigate('/singularity')}
                >
                  <Orbit className={`size-24 transition-colors duration-1000 ${isComplete ? 'text-purple-400 animate-float' : 'text-blue-400 animate-float'}`} />
                  <div className={`absolute -bottom-4 ${isComplete ? 'bg-magentaPulse' : 'bg-cyanNeon'} text-black text-[8px] font-black uppercase tracking-tighter px-4 py-1.5 rounded-full shadow-neonGlow whitespace-nowrap`}>
                    {isComplete ? 'Singularity Stabilized' : 'Eternal Heartbeat Active'}
                  </div>
                </motion.div>
              </div>
              <div className="mt-16 space-y-4">
                <h1 className="text-9xl font-black tracking-tighter uppercase italic text-gold drop-shadow-[0_10px_30px_rgba(0,0,0,0.5)]">Galaxy Core</h1>
                <div className="flex items-center justify-center gap-4">
                  <InfinityIcon className="size-4 text-blue-500 animate-pulse" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <CardContent className="p-4 h-48 overflow-hidden relative">
              <div className="space-y-2 font-mono text-[9px]">
                {state?.orchestrationLog?.slice(-10).map((log, i) => (
                   <div key={i} className={`flex items-center gap-3 ${log.includes('CRITICAL') || log.includes('SINGULARITY') ? 'text-purple-400 font-bold' : 'text-emerald-400/60'}`}>
          ...
          <Card className="glass-dark border-purple-500/20 bg-purple-500/5 text-white">
            ...
            <CardContent className="space-y-2">
              ...
              <Button onClick={() => navigate('/universe')} variant="ghost" className="h-6 text-[9px] uppercase font-black text-purple-400 w-full p-0 hover:bg-purple-500/10">Access Archive</Button>
            </CardContent>
          </Card>
        ...
        <div className="fixed bottom-12 left-1/2 -translate-x-1/2 w-full max-w-xl px-6 z-50">
           <div className={`glass-dark border p-5 rounded-full flex items-center gap-8 shadow-2xl transition-all duration-1000 ${isComplete ? 'border-purple-500/50 shadow-purple-500/20' : 'border-white/10'}`}>
              ...
              <Button
                ...
                className={`h-14 px-10 rounded-full text-xs font-black uppercase tracking-[0.2em] shadow-lg transition-all ${isComplete ? 'bg-purple-600 text-white shadow-purple-500/40' : 'bg-blue-600 hover:bg-blue-500 shadow-blue-500/40'}`}
              >
