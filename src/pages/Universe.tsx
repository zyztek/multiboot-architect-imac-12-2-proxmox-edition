...
import { Search, Sparkles, LayoutGrid, List, Plus, Activity, Zap, Cpu, Eye, WifiOff, Globe } from 'lucide-react';
...
export function Universe() {
  ...
  const allCodexItems = useMemo(() => {
    return [...(staticCodex ?? []), ...(state?.customCodex ?? [])];
      ...
      <div className="space-y-10 animate-fade-in pb-24">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 border-b border-white/10 pb-8">
          <div className="space-y-2">
            ...
            <p className="text-blue-500 font-mono text-[10px] tracking-[0.4em] uppercase">Eternal Registry • Sync {progress.toFixed(1)}%</p>
          </div>
        ...
        {(isStaticLoading || isStateLoading) ? (
          ...
          <div className={`grid gap-4 ${denseMode ? 'grid-cols-2 sm:grid-cols-4 lg:grid-cols-6' : 'grid-cols-1'}`}>
            {filtered.map((item) => (
              ...
              >
                <CardHeader className="p-4 flex flex-col space-y-3">
                ...
                <CardContent className="px-4 pb-4 space-y-3">
                  ...
                  <div className="flex gap-1.5">
                    {item.complexity === 'God' && <Badge className="bg-rose-600 text-[6px] h-4 uppercase">VR</Badge>}
                    {item.category === 'AI' && <Badge className="bg-blue-600 text-[6px] h-4 uppercase">Ollama</Badge>}
                    {item.isUnlocked && <Badge variant="outline" className="text-[6px] h-4 border-emerald-500/30 text-emerald-400 uppercase">Offline</Badge>}
                  </div>
                </CardContent>
              </Card>
            ))}
