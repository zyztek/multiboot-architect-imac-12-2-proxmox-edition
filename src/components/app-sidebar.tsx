...
export function AppSidebar(): JSX.Element {
  ...
  return (
    <Sidebar className="border-r border-white/5 glass-dark backdrop-blur-2xl text-slate-300">
                ...
                <SidebarMenuButton
                  ...
                  className={`
                    ...
                    ${location.pathname === item.path
                      ? (item.elite ? 'bg-rose-500/10 text-rose-400 border-rose-500/40 shadow-glow' : 'bg-blue-600/10 text-blue-400 border-blue-500/30')
                      : 'border-transparent hover:bg-white/5 hover:text-white'}
                  `}
                >
                  <Link to={item.path} className="flex items-center gap-3">
                    <item.icon className={`size-4 ${location.pathname === item.path ? (item.elite ? 'text-rose-400' : 'text-blue-400') : 'text-slate-500 group-hover:text-white'}`} />
                    <span className="font-bold tracking-tight text-xs uppercase">{item.label}</span>
                    {item.elite && (
                      <motion.div 
                        animate={{ scale: [1, 1.3, 1] }} 
                        transition={{ duration: 2, repeat: Infinity }}
                        className="ml-auto size-1.5 rounded-full bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.8)]" 
                      />
                    )}
                  </Link>
                </SidebarMenuButton>
